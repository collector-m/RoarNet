...
vector<tDetection> loadDetections(string file_name, bool &compute_aos,
  ...
  if (!fp) {
    // success = false; # ignore detection file which doesn't have any content in it.
    cout << "no object in this file" << file_name << endl;
    success = true;
    return detections;
  }
  while (!feof(fp)) {
...
bool eval(string result_sha){
  ...
  string gt_dir         = "../datasets/kitti/training/label_2";
  // string gt_dir         = "../" + result_sha + "/label_2";

  string result_dir     = "../" + result_sha;
  string gt_label_file  = "../datasets/validset.txt";
